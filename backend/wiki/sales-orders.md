...
2. **Take payment** — for checkout channels, record the payment now; it sits against the customer's account (they have paid ahead) until the goods ship.
...
**Invoice before shipping** exists for billing a wholesale buyer in advance; normally shipping does it. **Cancel** an order that has not shipped; if it holds a checkout payment, reverse (refund) that first. Invoices (/invoices), Shipped (/deliveries), Payments received (/sales-payments: every customer payment, and **Register payment** against any open invoice or unshipped order) and Customers (/catalogs/customers) are under Sales. PDFs: invoice, packing slip, 4×6 label.
